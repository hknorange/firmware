...
def get_port_device():
    with open('/root/wiringOP/code_220923_gateway/portusbcamera/port_usb_3_0.txt','r') as file:
        output = file.read()
    return output
...
if(cap.isOpened()):
    now = datetime.now()
    date_time = now.strftime('front_date_%d_%m_%Y time_%H_%M_%S')
...
def task2():
    ...
    while True:
        ...
        #display_frame = frame
        result.write(frame)
        tframe = imutils.resize(frame,width=320)
        message_dict = {
            'frame': tframe,
            'license_car': car_ip,
            'position_camera': '2'
